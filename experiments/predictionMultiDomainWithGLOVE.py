# EXPERIMENT: PREDICTION ON MULTI-LINGUAL (ENGLIGH) VOCABULARIES WITH GLOVE EMBEDDINGS
import os.path
import pickle
...
# LOAD VOCABULARIES, WHICH WILL BE USED FOR DOMAIN-SPECIFIC PREDICTIONS [TEST DATA]
multi_lingual_vocab_path = '../data/multi-lingual-data/en_am/all.vocab'  # Change this to switch to differet test set
vocab_list = list()
vocab_list_glove = list()
with open(multi_lingual_vocab_path) as f:
    for line in f:
        token = line.split()[1]
        vocab_list.append(token)
        if token in glove_embeddings and len(glove_embeddings[token]) == 300:
            vocab_list_glove.append(token)
...
if WRITE_CSV:
    print('\nWriting into CSV')
    sentiment_df.to_csv('../outputs/predicted_sentiments_am_with_GLOVE.csv')
